...
inline fn writeRegister(self: *AdlibInstance, reg_num: u16, value: u8) void {
    OPL3.OPL3_WriteRegBuffered(&self.opl_chip, reg_num, value);
}

pub fn init(self: *AdlibInstance, sample_rate: u32) Backend.Error!void {
...
pub fn fillBuffer(self: *AdlibInstance, buffer: []i16, nsamples: u32) void {
    if (nsamples > 0) {
        OPL3.OPL3_GenerateStream(&self.opl_chip, &buffer[0], nsamples);
    }
}

pub fn isPlayingATrack(self: *AdlibInstance) bool {
    return self.active_channels != 0;
}

pub fn playTrackNumber(self: *AdlibInstance, song_number: u8) void {
...
pub fn playTrack(self: *AdlibInstance, track: ?AudioTrack) void {
...
pub fn playSfx(self: *AdlibInstance, fx_number: u8) void {
    self.sfx_time = 15;
    self.sfx_on = true;
    const index: usize = @intCast(fx_number);
    AdlibInstance.insmaker(self, &self.sfx[index].op[0], 0x13); //Channel 6 operator 1
    AdlibInstance.insmaker(self, &self.sfx[index].op[1], 0x10); //Channel 6 operator 2
    self.writeRegister(0xC6, self.sfx[index].fb_alg); //Channel 6 (Feedback/Algorithm)
}

fn sfx_init(self: *AdlibInstance) void {
    self.sfx_on = false;
    self.sfx_time = 0;
...
fn sfx_driver(self: *AdlibInstance) void {
    if (!self.sfx_on) {
        return;
    }
...
fn sfx_stop(self: *AdlibInstance) void {
...
fn processInstruction(self: *AdlibInstance, channel: *Channel, instruction: Instruction) void {
...
fn fillchip_channel(self: *AdlibInstance, i: usize) void {
...
pub fn fillchip(self: *AdlibInstance) void {
    self.sfx_driver();
...
fn insmaker(self: *AdlibInstance, insdata: []const u8, channel: u8) void {
...
fn all_vox_zero(self: *AdlibInstance) void {
    for (0xB0..0xB9) |i|
