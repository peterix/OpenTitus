...
pub fn viewintrotext(_: std.mem.Allocator) !c_int {
    ...
    var timestamp: SDL.DateTime = .{};
    var buf: [64]u8 = undefined;

    if(!SDL.getCurrentTime(&ticks))
    {
        return error.CannotDetermineTime;
    }
    if(!SDL.timeToDateTime(ticks, &timestamp, true))
    {
        return error.CannotDetermineTime;
    }
    ...
    const year = try std.fmt.bufPrint(&buf,
                      "     You are still playing Moktar in {d} !!", .{timestamp.year});
